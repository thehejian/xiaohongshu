...
BASE="/Users/mac/.qclaw/workspace/002-AIagent/03-opencode/003-Twitter/naming-xhs"
api_key=''
...
def gen(name):
    ...
    if 'data' in d:
        ...
        print("naming",name,"OK",sz,"KB")
        return d['data'][0]['url']
    else:
        print("naming",name,"FAIL:",r.stdout[:200])
        return None
gen("01-cover")
ref_b64=base64.b64encode(open(BASE+"/01-cover.png","rb").read()).decode()
for name in ["02-jose","03-aureliano","04-fate","05-warning","06-cycle"]:
    p=open(f"{BASE}/prompts/{name}.md").read().strip()
    ...
    if 'data' in d:
        ...
        sz=os.path.getsize(f"{BASE}/{name}.png")//1024
        print("naming",name,"OK",sz,"KB")
    else: print("naming",name,"FAIL:",r.stdout[:200])
print("naming Done!")
