...
BASE="/Users/mac/.qclaw/workspace/002-AIagent/03-opencode/003-Twitter/kitano-xhs"
api_key=''
...
for name in ["02-manzai","03-director","04-accident","05-painter","06-ending"]:
    ...
    with open(f"/tmp/kt_{name}.json","w") as f: json.dump(payload,f)
    r=subprocess.run(["curl","-s","--max-time","300","-H","Authorization: Bearer "+api_key,"-H","Content-Type: application/json","-d",f"@/tmp/kt_{name}.json","https://apihub.agnes-ai.com/v1/images/generations"],capture_output=True,text=True,timeout=300)
