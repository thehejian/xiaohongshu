...
for name in ["02-manzai","03-tvchaos","04-friday","05-funeral","06-game"]:
    ...
    with open(f"/tmp/kt2_{name}.json","w") as f: json.dump(payload,f)
    r=subprocess.run(["curl","-s","--max-time","300","-H","Authorization: Bearer "+api_key,"-H","Content-Type: application/json","-d",f"@/tmp/kt2_{name}.json","https://apihub.agnes-ai.com/v1/images/generations"],capture_output=True,text=True,timeout=300)
