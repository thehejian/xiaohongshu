...
BASE="/Users/mac/.qclaw/workspace/002-AIagent/03-opencode/003-Twitter/banana-massacre-xhs"
api_key=''
...
def gen(name):
    ...
    if 'data' in d:
        ...
        print("bmass",name,"OK",sz,"KB")
        return d['data'][0]['url']
    else:
        print("bmass",name,"FAIL:",r.stdout[:200])
        return None
gen("01-cover")
ref_b64=base64.b64encode(open(BASE+"/01-cover.png","rb").read()).decode()
for name in ["02-strike","03-square","04-train","05-survivor","06-denial"]:
    p=open(f"{BASE}/prompts/{name}.md").read().strip()
    ...
    if 'data' in d:
        ...
        sz=os.path.getsize(f"{BASE}/{name}.png")//1024
        print("bmass",name,"OK",sz,"KB")
    else: print("bmass",name,"FAIL:",r.stdout[:200])
print("bmass Done!")
