...
BASE="/Users/mac/.qclaw/workspace/002-AIagent/03-opencode/003-Twitter/opening-ending-xhs"
api_key=''
...
def gen(name):
    ...
    if 'data' in d:
        ...
        print("oe",name,"OK",sz,"KB")
        return d['data'][0]['url']
    else:
        print("oe",name,"FAIL:",r.stdout[:200])
        return None
gen("01-cover")
ref_b64=base64.b64encode(open(BASE+"/01-cover.png","rb").read()).decode()
for name in ["02-ice","03-end","04-circle","05-author","06-book"]:
    p=open(f"{BASE}/prompts/{name}.md").read().strip()
    ...
    if 'data' in d:
        ...
        sz=os.path.getsize(f"{BASE}/{name}.png")//1024
        print("oe",name,"OK",sz,"KB")
    else: print("oe",name,"FAIL:",r.stdout[:200])
print("oe Done!")
