...
BASE="/Users/mac/.qclaw/workspace/002-AIagent/03-opencode/003-Twitter/aureliano-xhs"
api_key=''
...
for name in ["02-wars","03-goldfish","04-sons","05-quote","06-ending"]:
    ...
    with open(f"/tmp/al_{name}.json","w") as f: json.dump(payload,f)
    r=subprocess.run(["curl","-s","--max-time","300","-H","Authorization: Bearer "+api_key,"-H","Content-Type: application/json","-d",f"@/tmp/al_{name}.json","https://apihub.agnes-ai.com/v1/images/generations"],capture_output=True,text=True,timeout=300)
