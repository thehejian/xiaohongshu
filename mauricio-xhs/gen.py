...
BASE="/Users/mac/.qclaw/workspace/002-AIagent/03-opencode/003-Twitter/mauricio-xhs"
api_key=''
...
def gen(name):
    ...
    if 'data' in d:
        ...
        print("mc",name,"OK",sz,"KB")
        return d['data'][0]['url']
    else:
        print("mc",name,"FAIL:",r.stdout[:200])
        return None
gen("01-cover")
ref_b64=base64.b64encode(open(BASE+"/01-cover.png","rb").read()).decode()
for name in ["02-butterflies","03-roof","04-shot","05-paralyzed","06-death"]:
    p=open(f"{BASE}/prompts/{name}.md").read().strip()
    ...
    if 'data' in d:
        ...
        sz=os.path.getsize(f"{BASE}/{name}.png")//1024
        print("mc",name,"OK",sz,"KB")
    else: print("mc",name,"FAIL:",r.stdout[:200])
print("mc Done!")
