...
BASE="/Users/mac/.qclaw/workspace/002-AIagent/03-opencode/003-Twitter/worldcup32-xhs"
api_key=''
...
for name in ["02-asia","03-map","04-korea","05-history","06-ending"]:
    ...
    with open(f"/tmp/wc_{name}.json","w") as f: json.dump(payload,f)
    r=subprocess.run(["curl","-s","--max-time","300","-H","Authorization: Bearer "+api_key,"-H","Content-Type: application/json","-d",f"@/tmp/wc_{name}.json","https://apihub.agnes-ai.com/v1/images/generations"],capture_output=True,text=True,timeout=300)
